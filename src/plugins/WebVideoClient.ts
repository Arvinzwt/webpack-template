...
class VideoStream extends BaseEvent {
    ...
    display(videoElement: any) {
        ...
        if (this.localStream) {
            ...
            // 获取本地声音音量，并emit发送
            if(!this.localStream.intervalId) {
                clearInterval(this.localStream.intervalId);
                this.localStream.intervalId = setInterval(() => {
                    let level = Math.round(this.localStream.getAudioLevel() * 10);
                    this.emit('localAudioLevel', level)
                }, 100);
            }

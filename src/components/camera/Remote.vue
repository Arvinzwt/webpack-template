<template>
    <div class='jr-camera-teacher' @mouseenter="onMouseenter" @mouseleave="onMouseLeave">
        <!-- 远程视频流容器 -->
        <video class="remotevideo" :id=" 'remotevideo' + userId" playsinline autoplay v-show="isRemote"></video>
        

        <!-- 视频未接入/未显示，显示的缺省画面 -->
        <div class="video-item" v-show="!isRemote">
            <div class="video-item-placeholder" :class="urlQuery.role == 1 ? 'type1' : 'type2'"></div>
        </div>

        <!-- 右上角wifi信号，钻石数目 -->
        <div class="video-item-icon icon1">
        ...
        <!--声音状态-->
        <Volume ref="remote_volume" v-show="remoteInfo.user_id"></Volume>

        <!-- 左下角用户名 -->
        <div class="video-item-icon icon2" v-show="remoteInfo.user_id">
            <span class="text-ellipsis">{{ remoteInfo.info && remoteInfo.info.name }}</span>
        </div>

        <!-- private -->
        <!-- 老师，可以控制学生，魔法棒，禁言，画笔 -->
        <div class="video-item-icon opera-list opera-teacher"  v-if="isRemote && urlQuery.role == UserType.TEACHER">
        ...
        <div class="jr-parent-limit" v-if="urlQuery.role == UserType.ADMIN && isShowStudent">
            <div class="opera-list opera-student">
        ...
        <!-- 家长， 学生，右下角画笔/禁言 -->
        <div class="video-item-icon opera-list opera-student" v-if="urlQuery.role == UserType.PARENT && remoteInfo.role == UserType.STUDENT">
...
export default class Teacher extends Vue {
    ...
    */
    ...
    bind(id:string, type:number,  role?: number) {
        let videoClient = this.$videoClient;
        ...
        let token = `${type}${id}`
        videoClient.display(document.querySelector(`#remotevideo${token}`), token);
        
        // @ts-ignore
        // this.$refs['remote_volume'].init(stream);
    }

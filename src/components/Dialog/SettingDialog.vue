...
export default class Setting extends Vue {
    ...
    private onSureSet() {
        ...
        if(this.setStream.cameraId == this.$videoStream.cameraId && this.setStream.microphoneId == this.$videoStream.microphoneId) {
            console.warn('设备未切换, 直接关闭弹窗');
            this.onCloseSetting();
            return false;
        }
        ...
        this.$videoStream.createStream().then((stream: any) => {
            ...
            this.$api.saveDevice(this.$videoStream.cameraId, this.$videoStream.microphoneId)
                .then((res:any) => {
                    console.log('设备切换成功')
                })
            this.onCloseSetting();
